...
|---|---|---|
| Character Profile | A character's animations, per-frame hitboxes and sockets, behavior-carrying Frame Cues, curves, root motion, tags, and transition-map data | Project-wide gameplay state machines, attack playback orchestration, damage authority, or persistent actor state |
...
See [Character Layer Authoring](layer-authoring-guide.md), [Runtime Appearance](runtime-appearance-guide.md), [Frame Cues](frame-cues-guide.md), [Effect Profiles](effect-profile-guide.md), [Combat Profiles](combat-profile-guide.md), and [Character Catalogs](character-catalog-guide.md) for the complete workflows.
...
Paper2DPlus does not choose encounters, spawn Catalog characters, play Combat-selected attacks, apply damage, or turn Effect Profile membership into runtime behavior. Cue Types do execute their own frame-driven behavior, while each project still owns persistent game state, networking authority, and the gameplay policies around those callbacks.
