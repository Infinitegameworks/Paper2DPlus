...
The script deliberately does not pass `-nullrhi`. It runs only `Paper2DPlusProfile200.AppearanceCrowd.Render.Profile200`, checks the automation result and every hard gate, and leaves two artifacts:
...
The matrix copies the current plugin source into a dedicated scratch host root (`<host-test-root>`), removes only guarded
...
`<reports-root>\profile200\<run-id>\UE-<version>\`; `summary.json` and `summary.md`
